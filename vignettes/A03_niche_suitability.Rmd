...
The influence of environmental conditions on species distribution is a central topic in ecology.
...
Contrary to the above, some traits may not require to be stored at the population level, e.g. if they are a property of the species as a whole.
In this case, `population_level` can be set to `FALSE` and the trait will be added as-is, without further processing.
In this example, this applies to the environmental niche of the species.
We assume that the environmental preferences of a species are the same for all population and therefore do not need to be stored at the population level.

Therefore, we can give both species different environmental preferences for the two environmental variables in the simulation environment (temperature & precipitation).
...
It is the responsibility of the species processes (therefore of the user) to access the correct traits and use them in the correct way, which is why meaningful trait names are important.
```{r add_trait_3}
sim$add_traits(
    species = "species_1",
