...
`metaRange` is not only able to simulate single species, it can also simulate multiple species at a time.
In this vignette, we will expand the example from the previous vignette to show how to set up a multi-species simulation.
...
* The `current` environment contains a 2 dimensional matrix (i.e. only one / the current layer) for each of the sub-datasets in the source SDS. These matricies are stored and accessible under the same name as the respective data sets in the source SDS and they are stored in memory.
