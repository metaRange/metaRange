---
title: "02: Advanced setup"
author: "Fallert, S. and Cabral, J.S."
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{02: Advanced setup}
...
1. Using a "burn-in" period in the simulation, before the main simulation begins.
...
2. If the environment is static and doesn't undergo any change, the user may want to use the same environmental raster layer for all time steps.
3. Shortening or lengthening the number of time steps of the simulation.
...
In the same way described above, one can also pick the specific layer that are used to use and shorten the simulation length.
```{r change_num_time}
...
sim$add_process(
    ...
    process_fun = function() {
        ...
        self$species_names() # no species yet
    },
...
A possibly use case would be to conditionally end the simulation if all species are extinct.
```{r exit}
sim <- create_simulation(create_example_landscape())
sim$set_time_layer_mapping(c(1:6))
sim$add_species(name = "species_1")
