...
```
To further expand the example we have so far, we can introduce more processes around the interaction between the two species.
...
Both species will have the same value for their optimal (i.e. their preferred) environmental niche values, but species 2 will be able to tolerate a larger range compared to species 1.

```{r suitability1, fig.cap="Figure 1: Suitability curve for the temperature niches of species 1 and species 2."}
opt_temp <- 283
min_temp_sp1 <- 279
max_temp_sp1 <- 287
min_temp_sp2 <- 260
...
Now we can add these values as traits to the species and also add the processes introduced in the previous tutorials.
...
For simplicity, we will assume asymmetric competition, in which species 1 is the superior competitor.
This means that if both species occur in one grid cell of the landscape:
