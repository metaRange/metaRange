...
Each species object has two main characteristics: `traits` which can be (somewhat arbitrary) pieces of data that describe and store information about the species and `processes` which are functions that describe how the species interacts with itself, time, climate and other species.
...
In this example we only use a singular species and one environmental factor (habitat quality).
...
To simplify this introduction, we use an example landscape consisting only of habitat quality data, with 10 time steps (layer) that are all the same (i.e. no environmental change).
...
At this point one has to switch to the sytax of the [R6](https://r6.r-lib.org/articles/Introduction.html) package, that metaRange uses.
...
This means that the abundance we use as input here represents the inital state of the simulation.
...
Note that the above plot is not very interresting, since the abundance is the same for each population at the beginning of the simulation.
...
In the case two (or more) processes have the same priority, it is assumed that they are independent from each other and that their execution order doesn't matter.
...
To do so, we can use a built in function `ricker_reproduction_model()` that implements the "classic" Ricker reproduction model ([Ricker, W.E. (1954)](https://doi.org/10.1139/f54-039) compare also: [Cabral, J.S. and Schurr, F.M. (2010)](https://doi.org/10.1111/j.1466-8238.2009.00492.x)) [Ref. 1 & 2].
