...
```
The ability to disperse is a key trait for many species and can have a large impact on the population dynamics and te ability of a species to survive and adapt to a changing environment.
...
In other scietific fields this process is known as convolution and is for example often used in image processing and especially when "blurring" an image.
...
## Basic setup
First, we load the necesarry packages and create an example landscape.
```{r create_landscape}
library(metaRange)
library(terra)
set_verbosity(2)
