...
This concept is described in the "metabolic theory of ecology" (MTE) by Brown et al. (2004) [Ref: 1] which can be expressen in the following equation:
...
Lets say for example we estimate that a species (avg. bodyweigth = 1 gramm) has a reproduction rate of 1 and a carrying capacity of 100 at a temperature of 20 degree Celcius (293.15 degree Kelvin).
...
Instead of the reference temperature, we can now give a vector of differnt temperatures (or possibly body weights), for which we want to calculate the scaling.
